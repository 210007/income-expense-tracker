...
export default function TransactionsClient() {
  ...
  // IMPORTANT: do NOT compute this at render time as a "static" initial state
  // Read it inside useEffect so it only happens on the client.
  ...
  const missingCount = useMemo(() => {
    return rows.filter((r) => r.type === "expense" && (receiptCounts[r.id] ?? 0) === 0).length;
  }, [rows, receiptCounts]);
    ...
    <main className="p-6 max-w-3xl mx-auto">
      <div className="flex flex-col gap-3">
        <div>
          ...
          <p className="opacity-80 mt-1">
            {showMissingOnly ? `Showing missing receipts (${missingCount})` : "Add, review, and categorize"}
          </p>
        </div>

        <div className="grid grid-cols-2 gap-2">
          <a href="/" className="text-center bg-black text-white py-3 rounded font-medium">
            Dashboard
          </a>

          <a href="/transactions?missing=1" className="text-center border py-3 rounded font-medium">
            Missing receipts
          </a>

          <a href="/transactions" className="text-center border py-3 rounded font-medium">
            Transactions
          </a>
      ...
      <section className="mt-6 border p-4 rounded">
        ...
        <div className="grid gap-2">
          ...
          <input className="border p-2 rounded" type="date" value={txnDate} onChange={(e) => setTxnDate(e.target.value)} />
          ...
          <input className="border p-2 rounded" placeholder="e.g. Walmart" value={vendor} onChange={(e) => setVendor(e.target.value)} />
          ...
          <button className="bg-black text-white px-4 py-3 rounded font-medium mt-2" onClick={addTransaction}>
            Add
          </button>

          <button className="border px-4 py-3 rounded font-medium mt-1" onClick={load}>
            Reload list
          </button>
        </div>
      </section>
      ...
      <section className="mt-6">
        <h2 className="font-semibold mb-3">{showMissingOnly ? "Missing receipts" : "Recent"}</h2>
        ...
        {loading ? (
          ...
          <div className="grid gap-2">
            {displayedRows.length === 0 ? (
              <p className="opacity-80">{showMissingOnly ? "No missing receipts — you're clean." : "No transactions yet."}</p>
            ) : (
              displayedRows.map((r) => {
                ...
                return (
                  <a key={r.id} href={`/transactions/${r.id}`} className="block border rounded p-4 active:opacity-80">
                    ...
                    <div className="flex justify-between text-sm opacity-80 mt-2">
                      <div>{r.txn_date}</div>
                      <div>{(r.category || "Uncategorized") + " • " + (hasReceipt ? "Receipt ✅" : "No receipt")}</div>
                    </div>
                  </a>
                );
              })
